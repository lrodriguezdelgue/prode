...
Tocá New project. Ponele nombre (ej. scalonetta), elegí una contraseña para la base (guardala, aunque no la vas a usar en la app) y una región (la más cercana, ej. South America (São Paulo)).
Esperá ~2 minutos a que el proyecto se cree.
En el menú izquierdo, andá a SQL Editor → New query.
Abrí el archivo supabase.sql de este proyecto, copiá TODO su contenido, pegalo y tocá Run. Debería decir "Success".
Ahora andá a Project Settings (el engranaje) → API. Anotá dos cosas:
...
Creá un repo nuevo en https://github.com/new (privado o público, da igual).
Subí todos los archivos de este proyecto al repo. Si no usás git por consola, podés usar el botón "uploading an existing file" de GitHub y arrastrar la carpeta (sin node_modules ni .env.local).


PASO 3 — Publicar en Vercel
Entrá a https://vercel.com y registrate con tu cuenta de GitHub.

Tocá Add New → Project y elegí el repo que subiste.

Vercel detecta que es un proyecto Vite solo. Antes de tocar Deploy, abrí Environment Variables y agregá estas dos (las del Paso 1):
...
Tocá Deploy. En ~1 minuto te da una URL tipo https://scalonetta.vercel.app.

¡Esa URL es la que compartís con tus amigos! No necesitan cuenta de nada, entran y juegan.


PASO 4 — Arrancar
Entrá vos primero a la URL y registrate → quedás como admin 👑.
Probá: cargá un pick, recargá la página. Tiene que seguir ahí (ahora sí persiste de verdad).
Desde otro dispositivo / que un amigo entre y se registre. En la Tabla tienen que aparecer los dos.
Cuando todo esté ok, en Admin → Zona peligrosa → Resetear todo limpiás los datos de prueba antes del Mundial.
Pasá la URL al grupo. 🎉
