...
function AppInner(){
  ...
  useEffect(()=>{ (async()=>{ await refresh(); setReady(true); })(); },[refresh]);
  useEffect(()=>{
  ...
  if(!me) return <><Style/><div className="scl"><Auth users={users} onAuth={async(u)=>{ await refresh(); setMe(u); }}/></div></>;
      ...
      <div style={{ position:"relative", overflow:"hidden", background:`linear-gradient(135deg,${C.celeste},${C.celesteDeep})`, color:"#fff", padding:"18px 16px 16px" }}>
        ...
        <div style={{ position:"relative", display:"flex", justifyContent:"space-between", alignItems:"center", gap:10 }}>
          ...
          <div style={{ textAlign:"right" }}>
            ...
            <button onClick={()=>setMe(null)} style={{ background:"rgba(255,255,255,.2)", color:"#fff", border:"none", borderRadius:8, padding:"4px 10px", fontSize:12, fontWeight:700, cursor:"pointer", marginTop:4 }}>Salir</button>
          </div>
        </div>
      </div>
